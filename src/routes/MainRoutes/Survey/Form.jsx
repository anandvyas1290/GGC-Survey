...
export default function Form(props) {
    ...
    return (
        <div className="max-w-screen-xl mx-auto">
            <section className="grid grid-cols-2 gap-10 border rounded-xl md:p-10 border-gray-300">
                {props?.fieldsData?.map((field) => (
                    <Fragment key={field?.id}>
                        <Input
                            label={field?.label}
                            className="border-2 border-gray-400 rounded-xl !bg-gray-100 !py-4 !text-lg"
            ...
            <div className="relative flex justify-center my-20">
                {!completed ? (
                    <div className="flex items-center justify-center">
                        ...
                        />
                        <button
                            className="group flex gap-3 absolute top-[2%] left-[45%] border-2 text-xl font-semibold rounded-full px-4 py-3 text-white bg-green-400 hover:bg-green-500"
                            onClick={() => {
